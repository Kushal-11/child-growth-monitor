...
def _service_with_spies(monkeypatch):
    ...
    service.measurement_svc.process_image_with_estimation.return_value = (
        MeasurementOutput(
    ...
    def fake_muac(**kwargs):
        ...
        return MUACResult(12.5, WastingStatus.NORMAL, "estimated_from_landmarks", True)
...
def _assert_manual_height_reaches_all_consumers(service, muac_calls):
    ...
    service.who_data.get_median_weight_for_height.assert_not_called()
    assert service.nutrition_svc.compute_haz.call_args.args[2] == MANUAL_HEIGHT
    service.nutrition_svc.compute_whz.assert_not_called()
...
def test_manual_height_controls_every_downstream_calculation(monkeypatch, db_session):
...
def test_ml_weight_is_evidence_not_an_effective_measurement(monkeypatch, db_session):
    service, _ = _service_with_spies(monkeypatch)
    service.ml_svc.predict.return_value.estimated_weight_kg = 10.0

    result = _assess(service, db_session)

    assert result.measurement.effective_weight_kg is None
    assert result.measurement.weight_method == "unavailable"
    assert result.measurement.predicted_weight_kg == 10.0
    assert result.nutrition.whz_zscore is None
    assert any("direct scale weight" in warning for warning in result.warnings)


def test_uncalibrated_image_height_cannot_drive_haz_or_whz(monkeypatch, db_session):
    service, muac_calls = _service_with_spies(monkeypatch)
    result = service.assess(
        db=db_session,
        image_path="front.jpg",
        child_name="Camera evidence only",
        dob=date(2023, 7, 1),
        sex="F",
        as_of=date(2026, 7, 28),
    )
    ...
    assert result.measurement.predicted_height_cm == IMAGE_HEIGHT
    assert result.measurement.effective_height_cm is None
    assert result.measurement.height_method == "unavailable"
    assert result.nutrition.haz_zscore is None
    assert result.nutrition.whz_zscore is None
    service.nutrition_svc.compute_haz.assert_not_called()
    service.nutrition_svc.compute_whz.assert_not_called()
    assert muac_calls[0]["height_cm"] is None


def test_validated_reference_object_height_is_effective(monkeypatch, db_session):
    service, _ = _service_with_spies(monkeypatch)
    output = service.measurement_svc.process_image_with_estimation.return_value
    output.estimation_method = "reference_object"
    output.reference_object_detected = True
    result = service.assess(
        db=db_session,
        image_path="front.jpg",
        child_name="Metric image",
        dob=date(2023, 7, 1),
        sex="F",
        as_of=date(2026, 7, 28),
    )
    assert result.measurement.effective_height_cm == IMAGE_HEIGHT
    assert result.measurement.height_method == "reference_object"
    assert result.nutrition.haz_zscore == -1.0
...
def test_api_returns_effective_manual_height_and_uses_it_downstream(
    ...
    try:
        response = TestClient(app).post(
            ...
            files={
                "image": ("front.jpg", b"image", "image/jpeg"),
