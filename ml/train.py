...
def scale_features(X_train, X_val, X_cal=None):
    ...
    return (X_train_s.astype("float32"), X_cal_s.astype("float32"),
            X_val_s.astype("float32"), scaler)
...
def train_cascade(le, X_train, y_train, X_val, y_val):
    ...
    stage1.fit(
        ...
        epochs=100, batch_size=256, class_weight=cw_bin,
        callbacks=[
            tf.keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True,
                                              monitor="val_accuracy"),
            tf.keras.callbacks.ReduceLROnPlateau(patience=5, factor=0.5, verbose=0),
    ...
    stage2.fit(
        X_train[tr_mask], y_tr_stage2,
        validation_data=(X_val[va_mask], y_va_stage2),
        epochs=100, batch_size=128, class_weight=cw_stage2,
        callbacks=[
            tf.keras.callbacks.EarlyStopping(patience=10, restore_best_weights=True,
                                              monitor="val_accuracy"),
            tf.keras.callbacks.ReduceLROnPlateau(patience=5, factor=0.5, verbose=0),
    ...
    for thr in (0.40, 0.50, 0.60):
        ...
        print(f"    thr={thr:.2f}  SAM_recall|wasted={rec_sam:.3f}  "
...
def fit_classifier_calibration(wc_model, le, X_cal_s, y_cal):
    ...
    conformal = ConformalCalibrator.fit(
        probs_cal, y_cal, class_names=list(le.classes_), alpha=0.10,
    )
    return T, conformal
...
def main():
    ...
    # 70 / 15 / 15 split — calibration set must be disjoint from train+val
    from sklearn.model_selection import train_test_split
    X_train, X_tmp, yw_train, yw_tmp, yc_train, yc_tmp = train_test_split(
        X, y_weight, y_class, test_size=0.30, random_state=42, stratify=y_class
    )
    X_cal, X_val, yw_cal, yw_val, yc_cal, yc_val = train_test_split(
        X_tmp, yw_tmp, yc_tmp, test_size=0.50, random_state=42, stratify=yc_tmp
    )
    ...
    stage1_model, stage2_model, cascade_thr1, cascade_thr2 = train_cascade(
        le, X_train_s, yc_train, X_val_s, yc_val,
    )
